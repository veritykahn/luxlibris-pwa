// pages/admin/god-mode.js - GOD MODE WITH AUDIT LOG VIEWER, PHASE MANAGEMENT, DROPDOWN PROGRAM SELECTION, AND VOTING RESULTS
import { useState, useEffect } from 'react'
import Head from 'next/head'
import { db, authHelpers, dbHelpers } from '../../lib/firebase'
import { collection, getDocs, addDoc, updateDoc, deleteDoc, doc, setDoc, query, orderBy, limit, where } from 'firebase/firestore'
...
export default function GodModeWithPrograms() {
  ...
  useEffect(() => {
    if (isAuthenticated && showAuditLogs) {
      loadAuditLogs();
    }
  }, [auditFilter, auditTimeRange, isAuthenticated, showAuditLogs]);
...
3. Students start fresh with clean slate`);
  ...
  >
    🗑️ Clear Student Data & Start Teacher Selection
        ...
        {/* Add pulse and spin animation CSS */}
        <style jsx>{`
          @keyframes pulse {
            0%, 100% {
              transform: scale(1);
