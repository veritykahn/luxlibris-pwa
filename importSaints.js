// importSaints.js - Saints Import Script
import { db } from './lib/firebase.js';
import { doc, setDoc, writeBatch } from 'firebase/firestore';

// 🏆 YOUR SAINTS DATA - Replace this with your complete saints JSON
const saintsData = [
  {
    "id": "saint_001",
...
async function importSaints() {
  console.log('🏆 Starting saints import...');
  console.log(`📊 Total saints to import: ${saintsData.length}`);
  
  try {
    // Use batched writes for better performance (Firebase allows 500 operations per batch)
    const batchSize = 500;
    let batchCount = 0;
    
    for (let i = 0; i < saintsData.length; i += batchSize) {
      const batch = writeBatch(db);
      const batchSaints = saintsData.slice(i, i + batchSize);
      
      for (const saint of batchSaints) {
        // Use the saint's id as the document ID (keep saint_ prefix for clarity)
        ...
        batch.set(saintRef, {
          ...
          shortBlurb: saint.short_blurb,
          extraFact: saint.extra_fact,
          iconAsset: saint.icon_asset,
          
          // Achievement system data
          rarity: saint.rarity,
          luxlingsSeries: saint.luxlings_series,
          
          // Unlock system (we'll define this based on rarity)
          unlockCondition: determineUnlockCondition(saint),
      ...
      await batch.commit();
      batchCount++;
      console.log(`✅ Batch ${batchCount} completed (${batchSaints.length} saints)`);
    }
    
    console.log('🎉 ALL SAINTS IMPORTED!');
    console.log('📊 Summary:');
    console.log(`   • Total Saints: ${saintsData.length}`);
    console.log(`   • Rarity Breakdown:`);
    
    // Count by rarity
    const rarityCount = {};
    saintsData.forEach(saint => {
      rarityCount[saint.rarity] = (rarityCount[saint.rarity] || 0) + 1;
    });
    
    Object.entries(rarityCount).forEach(([rarity, count]) => {
      console.log(`     - ${rarity}: ${count} saints`);
    });
...
// 🎯 DETERMINE UNLOCK CONDITIONS BASED ON YOUR VISION DOCUMENT
function determineUnlockCondition(saint) {
  // Ultimate Achievement: Jesus Christ
  if (saint.id === 'saint_ultimate_001') {
    return 'books_completed_100'; // Ultimate: 100 books = Jesus Christ
  }
  
  // Grade Saints: Start/End of year (seasonal)
  if (saint.rarity === 'seasonal') {
    return 'grade_milestone'; // June 1st start + March 31st end
  }
  
  // Grade-Exclusive Saints: Based on student's actual grade level
  if (saint.rarity === 'grade_exclusive_4') {
    return 'grade_4_only'; // Only unlocks for Grade 4 students
  }
  
  if (saint.rarity === 'grade_exclusive_5') {
    return 'grade_5_only'; // Only unlocks for Grade 5 students
  }
  
  if (saint.rarity === 'grade_exclusive_6') {
    return 'grade_6_only'; // Only unlocks for Grade 6 students
  }
  
  if (saint.rarity === 'grade_exclusive_7') {
    return 'grade_7_only'; // Only unlocks for Grade 7 students
  }
  
  if (saint.rarity === 'grade_exclusive_8') {
    return 'grade_8_only'; // Only unlocks for Grade 8 students
  }
  
  // Streak Saints: Based on reading streaks
  if (saint.rarity === 'common' && saint.luxlings_series === 'Contemplative Cuties') {
    return 'reading_streak_7'; // 7-day streak = Common streak saints
  }
  
  if (saint.rarity === 'rare') {
    return 'reading_streak_30'; // 30-day streak = Rare streak saints
  }
  
  // Achievement Saints: Book milestone unlocks for ALL grades
  // These are the saints that unlock at 5, 10, 15, 20 book milestones
  // (Different from grade-exclusive saints)
  
  // Mary Apparitions (Legendary - special achievements)
  if (saint.rarity === 'legendary' && saint.luxlings_series === 'Mini Marians') {
    return 'special_achievement'; // Major milestones/feast days
  }
  
  // Common saints - available from start or early milestones
  if (saint.rarity === 'common') {
    return 'always_available'; // Base collection students start with
  }
