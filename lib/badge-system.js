...
export const getCurrentWeekOfYear = () => {
  const now = new Date();
  const september1 = new Date(now.getFullYear(), 8, 1); // September 1st
  if (now < september1) {
    september1.setFullYear(now.getFullYear() - 1);
  }
...
export const BADGE_CALENDAR = {
  ...
  39: { name: "Pilot Celebration", emoji: "🎉", xp: 125, type: "timer", description: "Perfect final week: 30+ minutes daily to celebrate pilot success" }
};
