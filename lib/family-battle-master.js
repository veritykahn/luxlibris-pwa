  ...
  try {
    ...
    return await runTransaction(db, async (transaction) => {
      ...
      if (week.isSunday) {
        ...
        const completedWeek = battle.completedWeek || battle.currentWeek || null;
        
        if (!completedWeek || (completedWeek.children.total === 0 && completedWeek.parents.total === 0)) {
          return {
      ...
      if (week.isNewWeek) {
        ...
        if (currentWeekNumber !== week.number) {
          ...
          if (battle.currentWeek && (battle.currentWeek.children.total > 0 || battle.currentWeek.parents.total > 0)) {
...
export const getBattleData = async (familyId) => {
  try {
    ...
    if (week.isSunday) {
      const completedWeek = battle.completedWeek || battle.currentWeek;
