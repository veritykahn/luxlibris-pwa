// lib/firebase.js - Complete Updated Version with Phase Management and Academic Year System
...
// ===== ENHANCED AUTOMATIC PHASE CHECKING WITH STUDENT CLEARING =====

// Update the existing checkAndUpdatePhases function to include student clearing
export const checkAndUpdatePhasesWithClearing = async () => {
  try {
    const config = await getSystemConfig();
    const now = new Date();
    const currentPhase = config.programPhase;
    
    console.log('🔍 Checking automatic phase transitions with clearing...', {
      currentPhase,
    ...
    // May 24th: RESULTS → TEACHER_SELECTION (automatic with student clearing)
      ...
      if (now >= teacherSelectionDate) {
        newPhase = 'TEACHER_SELECTION';
        shouldUpdate = true;
        shouldClearStudents = true; // CLEAR STUDENT DATA
        console.log('🗑️ Auto-transition: RESULTS → TEACHER_SELECTION (May 24th) with student clearing');
      }
    }
    ...
    if (shouldUpdate) {
      if (shouldClearStudents) {
        // Use the enhanced clearing function
        const result = await transitionToTeacherSelectionWithClearing();
        
        console.log(`✅ Phase automatically updated with student clearing: ${currentPhase} → ${newPhase}`);
        console.log(`🗑️ Cleared data for ${result.studentsCleared} students`);
        
        return { 
          updated: true, 
          oldPhase: currentPhase, 
          newPhase,
          studentsCleared: result.studentsCleared,
          clearedData: true
        };
      } else {
    ...
    console.error('❌ Error in automatic phase checking with clearing:', error);
