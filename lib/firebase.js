...
// Check if student needs grade progression - FIXED VERSION
export const checkGradeProgression = async (studentData) => {
  try {
    const currentYear = getCurrentAcademicYear();
    
    // If student has no academicYear field, they definitely need an update
    if (!studentData.academicYear) {
      console.log('📈 Student has no academicYear field - needs update');
      return {
        needsUpdate: true,
    ...
    // If student has no gradeHistory, they need an update
    const gradeHistory = studentData.gradeHistory || [];
    if (gradeHistory.length === 0) {
      console.log('📈 Student has no gradeHistory - needs update');
      return {
        needsUpdate: true,
        lastRecordedYear: studentData.academicYear,
        currentYear: currentYear,
        currentGrade: studentData.grade,
        suggestedGrade: studentData.grade < 8 ? studentData.grade + 1 : 8,
        shouldBeAlumni: studentData.grade >= 8
    ...
    // Check if last recorded year is different from current year
    const lastEntry = gradeHistory[gradeHistory.length - 1];
    const lastRecordedYear = lastEntry?.academicYear || studentData.academicYear;
    const needsUpdate = lastRecordedYear !== currentYear;
    
    if (needsUpdate) {
      console.log('📈 Student needs grade progression check:', {
    ...
    return {
      ...
      currentGrade: studentData.grade,
      suggestedGrade: studentData.grade < 8 ? studentData.grade + 1 : 8,
      shouldBeAlumni: studentData.grade >= 8 && needsUpdate
    };
  } catch (error) {
...
// NEW: Academic year aware student data function
        ...
        for (const schoolDoc of schoolsSnapshot.docs) {
          try {
            ...
            if (!studentSnapshot.empty) {
              ...
              // Filter bookshelf by academic year (NEW!)
              ...
              return {
                ...
                entityId: entityDoc.id,
                schoolId: schoolDoc.id,
                academicYear: currentYear, // NEW: Always include current academic year
                ...studentData,
                bookshelf: currentYearBookshelf, // NEW: Only current year books
                allBookshelf: allBookshelf, // NEW: Keep all books for archive access
...
// NEW: Academic year aware book adding function
export const addBookToBookshelfEntitiesWithYear = async (studentId, entityId, schoolId, bookId, format) => {
  try {
    ...
    const newBookProgress = {
      bookId: bookId,
      format: format,
      academicYear: currentYear, // NEW: Include academic year
      currentProgress: 0,
      rating: 0,
      notes: '',
      completed: false,
...
export const dbHelpers = {
  ...
  // NEW: Academic year system functions
