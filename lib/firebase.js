...
export const deleteParentAccount = async (uid, linkedStudents = []) => {
  try {
    ...
    // Step 2: Delete family document
    const familyRef = doc(db, 'families', uid);
    batch.delete(familyRef);
    
    // Step 3: Remove parent connection from linked students
    if (linkedStudents.length > 0) {
      try {
        // Search for students and remove parent connection
          ...
          for (const schoolDoc of schoolsSnapshot.docs) {
            ...
            for (const studentDoc of studentsSnapshot.docs) {
              if (linkedStudents.includes(studentDoc.id)) {
                ...
                console.log('✅ Removed parent connection from student:', studentDoc.id);
              }
            }
          }
        }
    ...
    await batch.commit();
    console.log('✅ Parent data deleted from Firestore');
    
    // Step 5: Delete Firebase Auth account (this must be done by the user themselves)
    const currentUser = auth.currentUser;
    if (currentUser && currentUser.uid === uid) {
      await deleteUser(currentUser);
      console.log('✅ Firebase Auth account deleted');
