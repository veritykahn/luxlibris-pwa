// lib/firebase.js - Complete Updated Version with Phase Management, Enhanced Manual Student Clearing, and Account Deletion
...
export const deleteParentAccount = async (uid, linkedStudents = []) => {
  try {
    console.log('🗑️ Starting parent account deletion process...');
    console.log('Parent UID:', uid, 'Linked students:', linkedStudents.length);
    
    const batch = writeBatch(db);
    
    // Step 1: Get family document to check how many parents are in the family
    const familyRef = doc(db, 'families', uid);
    ...
    if (familyDoc.exists()) {
      ...
      const parentsInFamily = familyData.parents || [];
      
      // Check if this is the only parent in the family
      if (parentsInFamily.length > 1) {
        isOnlyParent = false;
        // Get other parent UIDs (excluding the one being deleted)
        otherParentUids = parentsInFamily.filter(parentId => parentId !== uid);
        console.log(`👨‍👩‍👧‍👦 Family has ${parentsInFamily.length} parents. Other parents:`, otherParentUids);
      } else {
        console.log('👤 This is the only parent in the family');
      }
    }
    
    // Step 2: Delete parent document
    const parentRef = doc(db, 'parents', uid);
    batch.delete(parentRef);
    console.log('📝 Scheduled parent document deletion');
    
    // Step 3: Handle family document based on parent count
    if (isOnlyParent) {
      // Delete the entire family document if this is the only parent
      batch.delete(familyRef);
      console.log('🏠 Scheduled family document deletion (last parent)');
    } else {
      // Remove this parent from the family's parents array
      const updatedParents = otherParentUids;
      batch.update(familyRef, { 
        parents: updatedParents,
        lastModified: new Date()
      });
      console.log('👨‍👩‍👧‍👦 Scheduled removal from family parents array');
    }
    
    // Step 4: Handle linked students
    if (linkedStudents.length > 0) {
      try {
        // Search for students and update their parent connections
        ...
        for (const entityDoc of entitiesSnapshot.docs) {
          ...
          for (const schoolDoc of schoolsSnapshot.docs) {
            ...
            for (const studentDoc of studentsSnapshot.docs) {
              if (linkedStudents.includes(studentDoc.id)) {
                const studentData = studentDoc.data();
                
                if (isOnlyParent) {
                  // If this was the only parent, remove all parent connections
                  batch.update(studentDoc.ref, { 
                    parentConnected: false,
                    parentId: null,
                    connectedParentEmail: null,
                    familyId: null
                  });
                  console.log('✅ Removed all parent connections from student:', studentDoc.id);
                } else {
                  // If there are other parents, just remove this parent's email
                  // Keep parentConnected true and familyId intact
                  const connectedEmails = studentData.connectedParentEmails || [];
                  const updatedEmails = connectedEmails.filter(email => email !== uid);
                  
                  batch.update(studentDoc.ref, { 
                    connectedParentEmails: updatedEmails,
                    // Note: parentId might need to be updated to point to remaining parent
                    parentId: otherParentUids[0] || null, // Use first remaining parent as primary
                    lastModified: new Date()
                  });
                  console.log('✅ Updated parent connections for student:', studentDoc.id);
                }
              }
            }
          }
        ...
        console.warn('⚠️ Could not update student connections:', studentUpdateError);
        // Don't fail the whole deletion for this
      }
    }
    
    // Step 5: Commit all Firestore deletions/updates
    await batch.commit();
    console.log('✅ Parent data deletion/updates committed to Firestore');
    
    // Step 6: Delete Firebase Auth account (this must be done by the user themselves)
    const currentUser = auth.currentUser;
    if (currentUser && currentUser.uid === uid) {
      await deleteUser(currentUser);
      console.log('✅ Firebase Auth account deleted');
    ...
    return { 
      success: true,
      wasOnlyParent: isOnlyParent,
      remainingParents: otherParentUids,
      familyDeleted: isOnlyParent
    };
...
// Enhanced parent deletion with data export option
export const deleteParentAccountWithExport = async (uid, linkedStudents = [], exportData = true) => {
  try {
    let exportResult = null;
    
    // Step 1: Export data if requested
    if (exportData) {
      exportResult = await exportParentData(uid);
    }
    
    // Step 2: Get parent email for logging
    const parentRef = doc(db, 'parents', uid);
    const parentDoc = await getDoc(parentRef);
    const parentData = parentDoc.data();
    
    // Step 3: Log the deletion
    await logAccountDeletion('parent', uid, parentData.email || 'unknown');
    
    // Step 4: Perform the deletion with multi-parent support
    const deletionResult = await deleteParentAccount(uid, linkedStudents);
...
// Helper function to check if parent is the only one in family
export const checkParentFamilyStatus = async (parentUid) => {
  try {
    ...
    const familyData = familyDoc.data();
    const parentsInFamily = familyData.parents || [];
    const otherParents = parentsInFamily.filter(pid => pid !== parentUid);
    
    return {
      hasFamily: true,
      isOnlyParent: parentsInFamily.length <= 1,
      parentCount: parentsInFamily.length,
...
export const dbHelpers = {
  ...
  deleteParentAccount: deleteParentAccount,
  ...
  checkParentFamilyStatus: checkParentFamilyStatus  // ADD THIS LINE
}
