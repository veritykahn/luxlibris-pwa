...
export const syncFamilyBattleData = async (familyId, linkedStudents) => {
  try {
    ...
    if (isSunday) {
      // Sunday: Show LAST Monday-Saturday results (excluding today)
      weekEnd = new Date(today);
      weekEnd.setDate(weekEnd.getDate() - 1); // Saturday at 11:59 PM
      ...
      console.log('🙏 SUNDAY DAY OF REST - Showing Mon-Sat results for Week', weekNumber);
    } else {
      ...
      console.log('⚔️ Battle Week', weekNumber, 'in progress (Mon-Sat)');
    }
...
export const getStudentBattleContribution = async (studentData, weekStr = null) => {
  try {
    if (!weekStr) {
      ...
      let weekStart;
      if (isSunday) {
        // Sunday: Get last week's contribution (Monday-Saturday)
    ...
    // Get the end date for the week (Saturday, 6 days from Monday)
...
export const getStudentFamilyBattleStatus = async (studentData) => {
  try {
    ...
    if (isSunday && battleData) {
      ...
      // Use current week (Monday-Saturday)
