...
// Generate unique parent invite code for student - NOW WITH SCHOOL CODE
export const generateParentInviteCode = (schoolCode, firstName, lastInitial, grade) => {
  // Longer random suffix for better uniqueness (8 characters)
  const randomSuffix = Math.random().toString(36).substring(2, 10).toUpperCase();
  return `${schoolCode}-${firstName.toUpperCase()}-${lastInitial.toUpperCase()}${grade}-INVITE-${randomSuffix}`;
};

// Create parent invite code for student (call from student dashboard button)
export const createParentInviteCode = async (studentId, entityId, schoolId, schoolCode, firstName, lastInitial, grade) => {
  const inviteCode = generateParentInviteCode(schoolCode, firstName, lastInitial, grade);
