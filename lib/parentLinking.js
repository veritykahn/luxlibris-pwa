// lib/parentLinking.js - COMPLETE FIXED VERSION for adding children through settings
...
// FIXED: Link parent to student - now handles when parent already has a family
export const linkParentToStudent = async (parentId, inviteCode, validatedCodeInfo = null, existingFamilyId = null) => {
  try {
    console.log('🔗 Starting linkParentToStudent process...');
    console.log('Parent ID:', parentId);
    console.log('Invite Code:', inviteCode);
    console.log('Existing Family ID passed:', existingFamilyId);
    ...
    // CRITICAL FIX: Get current parent's family status
    ...
    const parentCurrentFamilyId = parentData.familyId;
    
    console.log('📋 Parent current family ID:', parentCurrentFamilyId);
    console.log('📋 Student current family ID:', foundStudent.familyId);

    let familyInfo;

    // CRITICAL: If parent already has a family, add student to that family
    if (parentCurrentFamilyId) {
      console.log('🏠 Parent already has family - adding student to existing family:', parentCurrentFamilyId);
      
      // Get the current family to ensure it exists
      const currentFamilyDoc = await getDoc(doc(db, 'families', parentCurrentFamilyId));
      if (!currentFamilyDoc.exists()) {
        throw new Error('Parent family not found');
      }
      
      const currentFamilyData = currentFamilyDoc.data();
      
      // Check if student is already in this family
      const existingStudentIds = currentFamilyData.linkedStudents?.map(s => 
        typeof s === 'string' ? s : s.studentId
      ) || [];
      
      if (!existingStudentIds.includes(foundStudent.id)) {
        // Add student to the parent's existing family
        ...
        await updateDoc(doc(db, 'families', parentCurrentFamilyId), {
          linkedStudents: arrayUnion(newStudentEntry),
          lastUpdated: new Date()
        });
        
        console.log('✅ Student added to parent\'s existing family');
      } else {
        console.log('✅ Student already in parent\'s family');
      }
      
      // Update student with the parent's family ID (CRITICAL for family battles)
      await updateDoc(doc(db, studentPath), {
        familyId: parentCurrentFamilyId,
        lastModified: new Date()
      });
      ...
      familyInfo = {
        ...
        joinedExistingParentFamily: true
      };
      
    } else if (existingFamilyId) {
      // Parent doesn't have family but we're told to use an existing one (account creation flow)
      console.log('🏠 Using provided existing family ID:', existingFamilyId);
      ...
      if (existingFamily.exists) {
        ...
        // Update student - they should already have the familyId, but ensure consistency
        await updateDoc(doc(db, studentPath), {
          familyId: existingFamily.familyId,
          lastModified: new Date()
        });
...
// CRITICAL FIX: Sync child with ALL family members
if (familyInfo.familyId && !familyInfo.isNewFamily) {
  try {
    console.log('🔗 Syncing child with all family members...');
    
    // Get the complete family document to find all parents
    const familyDoc = await getDoc(doc(db, 'families', familyInfo.familyId));
    if (familyDoc.exists()) {
      const familyData = familyDoc.data();
      
      // Get all parents in the family (check both linkedParents and parents arrays)
      const allParentIds = [
        ...(familyData.linkedParents || []),
        ...(familyData.parents || [])
      ];
      ...
      console.log('👨‍👩‍👧‍👦 Found family parents:', uniqueParentIds);
      
      // Update child's linkedParents to include ALL parents in family
      await updateDoc(doc(db, studentPath), {
        linkedParents: uniqueParentIds, // Replace entire array to ensure all parents are included
        lastModified: new Date()
      });
      
      console.log('✅ Updated child linkedParents with all family parents');
      
      // Update ALL parents' linkedStudents arrays to include this child
      for (const familyParentId of uniqueParentIds) {
        try {
          await updateDoc(doc(db, 'parents', familyParentId), {
            linkedStudents: arrayUnion(foundStudent.id),
            schoolIds: arrayUnion(foundStudent.schoolId),
            lastUpdated: new Date()
          });
          
          console.log(`✅ Added child to parent ${familyParentId} linkedStudents`);
        } catch (error) {
          console.error(`⚠️ Error updating parent ${familyParentId}:`, error);
          // Continue with other parents even if one fails
        }
      }
      
      console.log('🎉 Complete family sync completed!');
      
    } else {
      console.warn('⚠️ Family document not found for sync:', familyInfo.familyId);
    }
  } catch (error) {
    console.error('⚠️ Error during family sync:', error);
